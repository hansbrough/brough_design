import React from 'react';
import * as styles from './blogpost.module.css';
import { StaticImage } from "gatsby-plugin-image";
import Blog from '../../components/Blog';
import Container from '../../components/Container';
import Layout from '../../components/Layout/Layout';

const BlogPostPage = (props) => {
  const imgDirPath = '../../../static/blogs/huntington-visit/';

  return (
    <Layout>
      <div className={styles.root}>
        <Container>
          <div className={styles.blogContainer}>
            <Blog
              category={'Travel'}
              title={'Huntington Botanical Gardens'}
              image={'/blogs/huntington-visit/IMG_7260.JPG'}
              alt={''}
            >
              <div className={styles.content}>
                <p className={styles.excerpt}>
                  I recently stopped by the Huntington Gardens in Pasadena, CA for the first time after what seems like years of reading about the trips of others.
                  I was truly surprised by the size of the grounds and variety of plants across the many themed gardens!
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    placeholder="blurred"
                    src={`${imgDirPath}IMG_7292.JPG`}
                    alt={'Entrance Alley'}
                    height={700}
                  />
                  <figcaption>Entrance Alley</figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  The Huntington is like Disneyland for plant aficionados - complete with a 'Main Street".
                  Along both sides of this alley are many patios for small groups, families or just individuals to relax.
                  I thought it was a brilliant idea!
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit}
                    src={`${imgDirPath}IMG_7290.JPG`}
                    alt={'One of many alcove patios offering privacy'}
                    height={700}
                  />
                  <figcaption>One of many alcove patios offering privacy</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit}
                    src={`${imgDirPath}IMG_7288.JPG`}
                    alt={'Lots of treasures in the alcove patios'}
                    height={700}
                  />
                  <figcaption>Lots of treasures in the alcove patios</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit}
                    src={`${imgDirPath}IMG_7283.JPG`}
                    alt={'Large pot filled with many plants'}
                    height={700}
                  />
                  <figcaption>We all need a garden pot filled with plants like this one!</figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  At end of the entrance alley is a rill with paths on either side leading visitors toward the heart of the botanical gardens.
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7281.JPG`} alt={'View from the top of the rill'}
                    height={700}
                  />
                  <figcaption>View from the top of the rill</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7276.JPG`} alt={'View from the bottom of the rill'}
                    height={700}
                  />
                  <figcaption>View from the bottom of the rill</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7275.JPG`} alt={'Aloe and grass meadow near the entrance'}
                    height={700}
                  />
                  <figcaption>Aloe and grass meadow near the entrance</figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  Of course I was looking forward to the Desert Garden in a big way but I truly enjoyed walking through the rest of the gardens as well.
                </p>
                <p className={styles.blogParagraph}>
                  There was a lot to be inspired by in the Desert Garden.
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7073.JPG`} alt={''}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  There were magic spots hidden throughout the garden.
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7130.JPG`} alt={''}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7133.JPG`} alt={''}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  The collector in me found many plants to admire.
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7223.JPG`} alt={'Euphorbia grandialata'}
                    height={700}
                  />
                  <figcaption>Euphorbia grandialata</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7245.JPG`} alt={''}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  There's something to be said for mass planting.
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7268.JPG`} alt={''}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7258.JPG`} alt={'grid of golden barrel cactus and artichoke agave'}
                    height={700}
                  />
                  <figcaption>Grid of golden barrel cactus and artichoke agave</figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  Most of the Desert Garden feels like a "collectors garden" which is what you would expect of a botanical garden... a few of everything.
                  There are a few sections (like the above and below shots) that look as if a designer had some input.
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7248.JPG`} alt={'Snowy looking mammillaria species dot hillside'}
                    height={700}
                  />
                  <figcaption>Snowy looking mammillaria species dot hillside</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7265.JPG`} alt={'Notocactus magnificus'}
                    height={700}
                  />
                  <figcaption>Notocactus magnificus</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7135.JPG`} alt={'cleistocactus borzicactus'}
                    height={700}
                  />
                  <figcaption>Cleistocactus borzicactus</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7137.JPG`} alt={''}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7128.JPG`} alt={''}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7121.JPG`} alt={'Fouquieria columnaris "Boojum Tree"'}
                    height={700}
                  />
                  <figcaption>Fouquieria columnaris "Boojum Tree</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7120.JPG`} alt={'Close up of "Boojum Tree" leaves'}
                    height={700}
                  />
                  <figcaption>Close up of "Boojum Tree" leaves</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7262.JPG`} alt={'Boojum Tree catching the last rays of light'}
                    height={700}
                  />
                  <figcaption>Boojum Tree catching the last rays of light</figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  If I get the chance I'll plant a few of these Boojum trees in my front yard!
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7328.JPG`} alt={'Enchanted hour in a cactus grove'}
                    height={700}
                  />
                  <figcaption>Enchanted hour in a cactus grove</figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  It was mid November when I took these pictures which means lots of dramatic lighting was free for all.
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7329.JPG`} alt={'Yucca filifera'}
                    height={700}
                  />
                  <figcaption>Yucca filifera</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7126.JPG`} alt={'pachycereus pringlei'}
                    height={700}
                  />
                  <figcaption>pachycereus pringlei</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7324.JPG`} alt={'Oreocereus celsianus "Old man of the Andes"'}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7330.JPG`} alt={'A very tall Echinopsis terscheckii'}
                    height={700}
                  />
                  <figcaption>Oreocereus celsianus "Old man of the Andes"</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7236.JPG`} alt={'Aloe bainesi (Aloe barberae)'}
                    height={700}
                  />
                  <figcaption>Aloe bainesi (Aloe barberae)</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7318.JPG`} alt={''}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  There seemed to be a lot of intriguing, off limit paths such as the one above - not to mention several older parts of the Desert Garden closed for renovation.
                </p>

                <h2 className={styles.blogSubHeader}>Aloes</h2>

                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7099.JPG`} alt={'Aloe principis cuttings'}
                    height={700}
                  />
                  <figcaption>Aloe principis cuttings</figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  Each of these cuttings was around 4 ft long and could have made small trees! They were headed for the mulcher.
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7094.JPG`} alt={'Vary mature Aloe ferox X A. microstigma'}
                    height={700}
                  />
                  <figcaption>Vary mature Aloe ferox X A. microstigma</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7074.JPG`} alt={''}
                    height={700}
                  />
                  <figcaption></figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7091.JPG`} alt={'Aloe reitzii'}
                    height={700}
                  />
                  <figcaption>Aloe reitzii</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7199.JPG`} alt={'Aloe sabae'}
                    height={700}
                  />
                  <figcaption>Aloe sabae</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7171.JPG`} alt={'Looks like a very mature Aloe ferox'}
                    height={700}
                  />
                  <figcaption>Looks like a very mature Aloe ferox</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7188.JPG`} alt={'Aloe tomentosa X A. secundiflora'}
                    height={700}
                  />
                  <figcaption>Aloe tomentosa X A. secundiflora</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7150.JPG`} alt={'Aloe mitriformis "Mitre Aloe"'}
                    height={700}
                  />
                  <figcaption>Aloe mitriformis "Mitre Aloe"</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7198.JPG`} alt={'Aloe framesii'}
                    height={700}
                  />
                  <figcaption>Aloe framesii</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7141.JPG`} alt={'Aloe cryptopoda'}
                    height={700}
                  />
                  <figcaption>Aloe cryptopoda</figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  Always good to see a friend in the garden and compare it to my own specimens growing back home.
                </p>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7143.JPG`} alt={'Aloe cryptopoda in bloom'}
                    height={700}
                  />
                  <figcaption>Aloe cryptopoda in bloom</figcaption>
                </div>
                <div className={styles.imageContainer}>
                  <StaticImage
                    className={styles.heightFit} src={`${imgDirPath}IMG_7227.JPG`} alt={'Aloe tororoana in bloom'}
                    height={700}
                  />
                  <figcaption>Aloe tororoana in bloom</figcaption>
                </div>
                <p className={styles.blogParagraph}>
                  Aloe tororoana is completely new to me. What caught my attention was the red blooms and thin, dark, reddish brown stems. Had they not been in bloom I probably would have walked right past them.
                </p>
                <p className={styles.blogParagraph}>
                  I took a lot of pictures - but too much of a good thing can get old. I have to admit that after walking through the gardens for hours I felt a bit overwhelmed. There is definately enough to keep you busy for an entire day - but you might be exausted at the end. I broke my visit into 2 half days - not on purpose but it ended up being a happy accident.
                </p>
                <p className={styles.blogParagraph}>
                  Until next time - hans
                </p>
              </div>
            </Blog>
          </div>
        </Container>
      </div>
    </Layout>
  );
};

export default BlogPostPage;
